...
app.post('/download', async (req, res) => {
  ...
  const history = fs.readJSONSync(HISTORY_FILE); // 读取当前历史记录（仅声明一次）
  ...
  // 初始化进度和历史记录（直接使用已声明的history变量，无需重复读取）
  progressMap.set(taskId, { percent: 0, speed: 0, status: '下载中' });
  history.push({ // 这里直接使用已存在的history变量，避免重复声明
    id: taskId,
